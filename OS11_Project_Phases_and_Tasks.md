...
- [x] Create `src/main/services/attachment/AttachmentService.ts`
...
- [x] Local file picker: `dialog.showOpenDialog({ properties: ['openFile', 'multiSelections'] })` → `AttachmentService.upload()`
- [x] Drag file onto task card: file dropped on `@dnd-kit` task drop target → IPC `ATTACHMENTS.UPLOAD`
- [x] Clipboard paste: `Ctrl+V` while a task is focused → check clipboard for `image/png` or `image/jpeg` → write temp file → upload
- [x] Cloud storage links: Google Drive / Dropbox / OneDrive — stored as URL reference in `attachments.local_path` with `is_link = 1` flag (requires `0003_attachment_links.sql` migration if not in initial schema)

### Attachment Display

- [x] Create `src/renderer/features/attachments/AttachmentStrip.tsx` + `.module.css`
...
- [x] On task card in list view: small paperclip icon + count only — no thumbnail (FEEL UI)
- [x] Attachment search: filenames indexed in command palette → type filename → jump to task

### Governance Update

- [x] CHANGELOG_INTERNAL.md: "Phase 15 complete: File attachments — all upload sources, thumbnail generation, display, clipboard paste."
- [x] Commit: `"Phase 15 complete: File attachments"`
